...
function gen_stock_count_id()
{
    ...
    $query = "SELECT `id` FROM `stock_update_ref` ORDER BY `id` DESC LIMIT 1";
...
if(isset($_POST['submit']))
{
    ...
    $squantity[] = $_POST['squantity']; //quanitty from database 
    $mquantity = $_POST['mquantity']; // entered quantity
    $difference[] = $_POST['diff']; //now quantity
    ...
    if($items == 0)
    {
        $warning = "Stock Addition Was not made. You did not enter any items";
    }
    ...
    else
    {
        ...
        for($i = 0; $i < $number; $i++)
        {
            ...
            if(!empty($index))
            {
                ...
                $mquant = filter($mquantity[$i]); // the quantity to be added
                $squant = filter($squantity[0][$i]); // the current or previous quantit
                $mdiff = filter($difference[0][$i]); // the new quantity
                
                //insert into the database;
                $query  = "INSERT INTO `stock_update` ("
                        . " `id`, `ref`, `date`, `product_code`, `product_name`, "
                        . " `old_quantity`, `added_quantity`, `new_quantity`, "
                        . " `day`, `month`, `year`, `time_added`, `user_id`)"
                        . ""
                        . " VALUE( "
                        . " 0, '$ref', '$date', '$mcode', '$mname',  "
                        . " '$squant', '$mquant', '$mdiff', '$day', "
                        . " '$month', '$year', NOW(), '$user_id')"; echo '<br>';
                $result = mysqli_query($dbc, $query)
                ...
                $comment = "Product Stock Added during <br>"
                        . " Stock Update Ref - " . $ref;
                
                //track (record the movement)
                move_inventory($mcode, $init, $end, $comment);
                
            }
        }
        //now insert into sales referece table
        $query = "INSERT INTO `stock_update_ref` ("
...
<div class="row">
    <div class="row">
        <div class="col-md-12">
            <h1 class="page-header"> Stock Addition</h1>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-8 col-md-offset-2">
            <div class="callout callout-info">
                <strong> Tip! </strong>
                This part is used when you have just purchase items and you want to update your inventory
            </div>
        </div>
    </div>
            ...
            <div >
                <table id="tb">
                    <tr>
                        ...
                        <th style="text-align: center;"> Curr. Quantity</th>
...
<div class="row">
    ...
    <div class="btn-group col-md-4" style="float: right;">
        <div class="btn-group">
            <input  class="btn btn-success" style="margin-right: 10px;" type="submit" value="Add  Stock" name="submit">
